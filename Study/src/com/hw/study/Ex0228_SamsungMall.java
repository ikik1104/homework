...
public class Ex0228_SamsungMall {
	public static void main(String[] args) {
		/*
		 * 	삼성가전제품 구매 프로그램(다형성을 이용하여) 20.02.28
		 * 
		 * 구매자, 
		 * 
		 * 
		...
		System.out.println("8)보유금액 충전            9)환불            10) 검색         0) 종료");
		System.out.println("---------------------------------------------");
	...
	void buy_list() {
		...
		for (int i = 0; i < cnt; i++) { //구매 개수만큼 출력되도록
			if(!(cart[i].name.equals(""))){
				System.out.print((i+1)+"."+cart[i].name+"\t"); //구매한 리스트의 이름 출력
			}
		}
		System.out.println();
		System.out.println("총 제품 구매개수 :"+(cnt-refund)+"개");
